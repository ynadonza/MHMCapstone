<?php 
 ...
 $query= "SELECT * FROM products WHERE sub_category = 'Defibrillators'";
...
$result = mysqli_fetch_array($data);
		...
		if($result['sub_category'] == "Defibrillators"){
			$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE prod_name like '%$search_value%' AND sub_category='Defibrillators' ORDER BY prod_id DESC limit $start_from, $record_per_page") or die(mysqli_error());
			...
			<div class="rsidebar span_1_of_left">
				 <section  class="sky-form">						   
					<div class="product_right">
						<h5 class="m_2"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span>Sub-Categories</h5>
						<div class="nav">
						<ul class="nav navbar place">	

								<li><a href="userglucosemonitors.php">Glucose Monitors</a></li>
								<li><a href="userphysicaltherapy.php">Physical Therapy Devices</a></li>
								<li><a href="userdefibrillators.php">Defibrillators</a></li>
								<li><a href="userbiomedicaltesting.php">Biomedical Testing Devices</a></li>
								<li><a href="userendoscopyequipment.php">Endoscopy Equipment</a></li>
							<div class="clearfix"> </div>
						</ul>
...
<center>
			<div class="pagination">
						<?php 
							if($sub_category = "Defibrillators")
							$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE sub_category = 'Defibrillators'") or die(mysqli_error());
							...
							if ($type='') {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "userdefibrillators.php?page=".($page-1);
								}
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="userdefibrillators.php?page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "userdefibrillators.php?page=".($page+1);
								} else {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "userdefibrillators.php?sub_category=".$_GET['sub_category']."&page=".($page-1);
								}
								echo '<li class="page-item '.$disable_prev.'"><a class="page-link" href="'.$prev_url.'" class="button">
							        <span aria-hidden="true">&laquo;</span>
							        <span class="sr-only">Previous</span>
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="userdefibrillators.php?sub_category='.$sub_category.'&page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "userdefibrillators.php?sub_category=".$_GET['sub_category']."&page=".($page+1);
								} else {
								    $disable_next = 'disabled';
								    $next_url = "javascript:void(0);";
								}
