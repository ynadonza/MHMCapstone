<?php 
 ...
 $query= "SELECT * FROM products WHERE sub_category = 'Surgical kits'";
...
$result = mysqli_fetch_array($data);
		...
		if($result['sub_category'] == "Surgical kits"){
			$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE prod_name like '%$search_value%' AND sub_category='Surgical kits' ORDER BY prod_id DESC limit $start_from, $record_per_page") or die(mysqli_error());
...
<div class="product-model">	 
	 <div class="container">
		  ...
		  <li class="active">Surgical Kits</li>
		 </ol>
			<h2>OUR PRODUCTS</h2>			
...
<center>
			<div class="pagination">
						<?php 
							if($sub_category = "Surgical kits")
							$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE sub_category = 'Surgical kits'") or die(mysqli_error());
							...
							if ($type='') {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "usersurgkits.php?page=".($page-1);
								}
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="usersurgkits.php?page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "usersurgkits.php?page=".($page+1);
								} else {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "usersurgkits.php?sub_category=".$_GET['sub_category']."&page=".($page-1);
								}
								echo '<li class="page-item '.$disable_prev.'"><a class="page-link" href="'.$prev_url.'" class="button">
							        <span aria-hidden="true">&laquo;</span>
							        <span class="sr-only">Previous</span>
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="usersurgkits.php?sub_category='.$sub_category.'&page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "usersurgkits.php?sub_category=".$_GET['sub_category']."&page=".($page+1);
								} else {
								    $disable_next = 'disabled';
								    $next_url = "javascript:void(0);";
								}
