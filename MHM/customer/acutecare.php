<?php 
 ...
 $query= "SELECT * FROM products WHERE type = 'acuteCare'";
...
?>
<?php
include("functions/acutecareDisplay.php");
?>	
<body>
...
<center>
			<div class="pagination">
						<?php 
							if($type = "Acute Care");
							$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE type = 'Acute Care'") or die(mysqli_error());
							...
							if ($type=='') {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "acutecare.php?page=".($page-1);
								}
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="acutecare.php?page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "acutecare.php?page=".($page+1);
								} else {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "acutecare.php?type=".$_GET['type']."&page=".($page-1);
								}
								echo '<li class="page-item '.$disable_prev.'"><a class="page-link" href="'.$prev_url.'" class="button">
							        <span aria-hidden="true">&laquo;</span>
							        <span class="sr-only">Previous</span>
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="acutecare.php?type='.$type.'&page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "acutecare.php?type=".$_GET['type']."&page=".($page+1);
								} else {
								    $disable_next = 'disabled';
								    $next_url = "javascript:void(0);";
								}
