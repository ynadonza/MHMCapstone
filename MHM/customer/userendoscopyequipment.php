<?php 
 ...
 $query= "SELECT * FROM products WHERE sub_category = 'Endoscopy'";
...
$result = mysqli_fetch_array($data);
		...
		if($result['sub_category'] == "Endoscopy"){
			$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE prod_name like '%$search_value%' AND sub_category='Endoscopy' ORDER BY prod_id DESC limit $start_from, $record_per_page") or die(mysqli_error());
...
<div class="product-model">	 
	 <div class="container">
		  ...
		  <li class="active">Endoscopy Equipment</li>
		 </ol>
			<h2>OUR PRODUCTS</h2>			
...
<center>
			<div class="pagination">
						<?php 
							if($sub_category = "Endoscopy")
							$sqlSelProd = mysqli_query($conn,"SELECT * FROM products WHERE sub_category = 'Endoscopy'") or die(mysqli_error());
							...
							if ($type='') {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "userendoscopyequipment.php?page=".($page-1);
								}
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="userendoscopyequipment.php?page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "userendoscopyequipment.php?page=".($page+1);
								} else {
								...
								$prev_url = "javascript:void(0);";
								    ...
								    $disable_prev = '';
								    $prev_url = "userendoscopyequipment.php?sub_category=".$_GET['sub_category']."&page=".($page-1);
								}
								echo '<li class="page-item '.$disable_prev.'"><a class="page-link" href="'.$prev_url.'" class="button">
							        <span aria-hidden="true">&laquo;</span>
							        <span class="sr-only">Previous</span>
								...
								for ($i=1; $i <=$total_pages ; $i++) { 
									echo '<li><a href="userendoscopyequipment.php?sub_category='.$sub_category.'&page='.$i.'">'.$i.'</a></li>';
								}
								if($page+1 == $total_pages) {
								    $disable_next = '';
								    $next_url = "userendoscopyequipment.php?sub_category=".$_GET['sub_category']."&page=".($page+1);
								} else {
								    $disable_next = 'disabled';
								    $next_url = "javascript:void(0);";
								}
